 ...
 * <p>
 * Current implementation is more or less a copy of the old xrootd code. Should be replaced by the
 * equivalent component developed by Tatjana and Tigran.
 */
public class XrootdDoor
     ...
     * <p>
     * Notice that the getter uses a different property name than the setter. This is because the
    ...
    public void setExecutor(ScheduledExecutorService executor) {
        ...
        executor.scheduleAtFixedRate(
              new FireAndForgetTask(new PingMoversTask<>(_transfers.values())),
              ...
              new XrootdTransfer(_pnfs, subject, restriction, path, opaque) {
                  ...
                  public synchronized void finished(CacheException error) {
                      ...
                      if (error == null) {
                          ...
                          _log.info("Transfer {}@{} failed: {} (error code={})", getPnfsId(),
                                getPool(), message, rc);
                      }
                  }
              };
    ...
    write(InetSocketAddress client, FsPath path, Set<String> tried,
        ...
        FsPath transferPath = persistOnSuccessfulClose ?
              getUploadPath(subject, restriction, createDir, overwrite, size, path, rootPath)
              : path;
     ...
     * @param createParents Indicates whether the parent directories of the directory should be
    ...
    /**
     * Emulate a file-move-operation by renaming sourcePath to targetPath in the namespace
     *
    ...
    /**
     * Encapsulate the list directory callback into a handler that manages the scheduled executor
     * service for the timeout handling.
     */
    private class DirlistRequestHandler {
        ...
        /**
         * Final listing result. Report back via callback and cancel the timeout handler.
         *
        ...
        /**
         * Partial listing result, report that back to the callback. Also, reset the timeout timer
         * in anticipation of further listing results.
         *
        ...
        /**
         * Remove the request handler from the list, report a failure to the callback and cancel the
         * timeout timer.
         *
        ...
        /**
         * Reschedule the timeout task with the same timeout as initially. Rescheduling means
         * cancelling the old task and submitting a new one.
         *
    ...
    /**
     * Check whether the given path matches against a list of allowed write paths.
     *
    ...
    /**
     * Check whether the given path matches against a list of allowed read paths.
     * <p>
     * Package visibility because needed by redirect handler in case of a third-party source
     * request.
     *
     * @param path the path which is going to be checked
     */
    ...
    /**
     * Try to find callback registered in listPath(...) and process the response there
     *
    ...
    public Set<Checksum> getChecksums(FsPath fullPath, Subject subject, Restriction restriction)
          throws CacheException {
        PnfsHandler pnfsHandler = new PnfsHandler(_pnfs, subject, restriction);
    ...
    public FileStatus getFileStatus(FsPath fullPath, Subject subject,
        ...
        FileAttributes attributes = pnfsHandler.getFileAttributes(fullPath.toString(),
              requestedAttributes);
